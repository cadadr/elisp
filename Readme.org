...
* ox-hugo setup
This readme file doubles as a source for the website for this repo,
using Kaushal Modi's [[https://github.com/kaushalmodi/ox-hugo][ox-hugo]] package.  Here is the setup for that.

The export destination is [[file:~/co/elisp-www/]]:

#+HUGO_BASE_DIR: ~/co/elisp-www

It's necessary to ~(require 'ox-hugo)~ beforehand.  Then run =C-c C-e
H A=.
...
#+INCLUDE: "./Readme.org::#introduction" :only-contents t

#+INCLUDE: "./Readme.org::#whatshere" :only-contents t

#+INCLUDE: "./Readme.org::#contributing" :only-contents t
